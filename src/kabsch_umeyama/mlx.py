...
def kabsch(P: mx.array, Q: mx.array) -> Tuple[mx.array, mx.array]:
    """
    ...
    d = R00 * (R11 * R22 - R12 * R21) - R01 * (R10 * R22 - R12 * R20) + R02 * (R10 * R21 - R11 * R20)
    
    # Correction
    d_sign = mx.where(d < 0, mx.array(-1.0, dtype=P.dtype), mx.array(1.0, dtype=P.dtype))
    ...
    t = mx.squeeze(centroid_Q, -2) - mx.squeeze(mx.matmul(centroid_P, R.swapaxes(-1, -2)), -2)

    return R, t


def kabsch_umeyama(P: mx.array, Q: mx.array) -> Tuple[mx.array, mx.array, mx.array]:
    """
    ...
    S_corr = mx.concatenate([mx.ones((*S.shape[:-1], D - 1), dtype=P.dtype), mx.expand_dims(d_sign, -1)], axis=-1)
    
    c = mx.sum(S * S_corr, axis=-1) / mx.maximum(var_P, 1e-12)

    centroid_P_rot = mx.matmul(centroid_P, R.swapaxes(-1, -2))
    t = mx.squeeze(centroid_Q, -2) - mx.expand_dims(c, -1) * mx.squeeze(centroid_P_rot, -2)
