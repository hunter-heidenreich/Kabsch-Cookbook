...
def kabsch(P: np.ndarray, Q: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    # Optimal rotation
    R = np.matmul(Vt.transpose(0, 2, 1) * np.stack([np.ones_like(d_sign), np.ones_like(d_sign), d_sign], axis=-1)[:, np.newaxis, :], U.transpose(0, 2, 1))
...
def kabsch_umeyama(P: np.ndarray, Q: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """
    ...
    Returns:
        (R, t, c, rmsd): Optimal rotation (Bx3x3), translation (Bx3), scale factor (B), and RMSD.
    """
    is_single = P.ndim == 2
    if is_single:
        P = P[np.newaxis, ...]
    ...
    # Optimal translation
    t = centroid_Q.squeeze(1) - c[:, np.newaxis] * np.squeeze(np.matmul(centroid_P, R.transpose(0, 2, 1)), 1)

    # RMSD
    aligned_P = c[:, np.newaxis, np.newaxis] * np.matmul(P, R.transpose(0, 2, 1)) + t[:, np.newaxis, :]
