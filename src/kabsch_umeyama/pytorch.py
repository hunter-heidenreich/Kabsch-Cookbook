...
class SafeSVD(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, A: torch.Tensor, eps: float = 1e-12) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        # Fast path 
    ...
    @staticmethod
    def backward(ctx, grad_U: torch.Tensor, grad_S: torch.Tensor, grad_V: torch.Tensor) -> tuple[torch.Tensor, None]:
...
def safe_svd(A: torch.Tensor, eps: float = 1e-12) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """
...
def kabsch(P: torch.Tensor, Q: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """
    ...
    # Safe SVD
    U, S, V = safe_svd(H)  # Bx3x3, Bx3, Bx3x3
    ...
    rmsd = torch.sqrt(torch.clamp(torch.sum(torch.square(aligned - q), dim=(1, 2)) / P.shape[1], min=1e-12))
...
def kabsch_umeyama(P: torch.Tensor, Q: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    """
    ...
    # Translation
    t = centroid_Q.squeeze(1) - c.unsqueeze(-1) * torch.matmul(centroid_P, R.transpose(1, 2)).squeeze(1)
    
    # RMSD
    aligned_P = c.unsqueeze(-1).unsqueeze(-1) * torch.matmul(P, R.transpose(1, 2)) + t.unsqueeze(1)
    rmsd = torch.sqrt(torch.clamp(torch.sum(torch.square(aligned_P - Q), dim=(1, 2)) / N, min=1e-12))
