...
def _safe_svd_bwd(res, g):
    ...
    grad_U = jnp.zeros_like(U) if type(grad_U) is jax.custom_derivatives.SymbolicZero else grad_U
    grad_S = jnp.zeros_like(S) if type(grad_S) is jax.custom_derivatives.SymbolicZero else grad_S
    grad_V = jnp.zeros_like(V) if type(grad_V) is jax.custom_derivatives.SymbolicZero else grad_V

    # V in JAX SVD is already V^T (like PyTorch Vh) if full_matrices is false, wait!
    # jnp.linalg.svd returns U, S, Vh. The original math was derived assuming A = U @ S @ Vh.
    ...
    # 4. J and K
    mH = lambda x: jnp.swapaxes(jnp.conj(x), -1, -2)
...
def _bwd(res, g):
    ...
    grad_U = jnp.zeros_like(U) if type(grad_U) is jax.custom_derivatives.SymbolicZero else grad_U
    grad_S = jnp.zeros_like(S) if type(grad_S) is jax.custom_derivatives.SymbolicZero else grad_S
    grad_V = jnp.zeros_like(Vh) if type(grad_V) is jax.custom_derivatives.SymbolicZero else grad_V
...
def kabsch(P: jnp.ndarray, Q: jnp.ndarray) -> Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray]:
    is_single = P.ndim == 2
    if is_single:
        P = P[jnp.newaxis, ...]
        Q = Q[jnp.newaxis, ...]
...
def kabsch_umeyama(P: jnp.ndarray, Q: jnp.ndarray) -> Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray]:
    is_single = P.ndim == 2
    if is_single:
        P = P[jnp.newaxis, ...]
        Q = Q[jnp.newaxis, ...]
    ...
    t = jnp.squeeze(centroid_Q, axis=1) - c[:, jnp.newaxis] * jnp.squeeze(jnp.matmul(centroid_P, jnp.swapaxes(R, 1, 2)), axis=1)
    
    aligned_P = c[:, jnp.newaxis, jnp.newaxis] * jnp.matmul(P, jnp.swapaxes(R, 1, 2)) + t[:, jnp.newaxis, :]
    rmsd = jnp.sqrt(jnp.clip(jnp.sum(jnp.square(aligned_P - Q), axis=(1, 2)) / N, min=1e-12, max=None))
